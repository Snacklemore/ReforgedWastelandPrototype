...
class WST_AIKillTaskSupportEntityClass: SCR_EditorTaskSupportEntityClass
{
};

//------------------------------------------------------------------------------------------------
class WST_AIKillTaskSupportEntity : SCR_EditorTaskSupportEntity 
{
	...
	WST_AIKillTask GetTask(Faction targetFaction, SCR_CampaignTaskType type)
	{
		if (!GetTaskManager())
			return null;
		...
		{
			WST_AIKillTask castTask = WST_AIKillTask.Cast(tasks[i]);
	...
	void RPC_SetTargetDestination(int taskID, vector destination)
	{
		...
		WST_AIKillTask task = WST_AIKillTask.Cast(GetTaskManager().GetTask(taskID));
		if (!task)
			return;
	...
	//------------------------------------------------------------------------------------------------
	void SetTargetDestination(notnull WST_AIKillTask task,  vector destination)
	{
		if (!GetTaskManager())
			return;
	...
	void SetIndividual(notnull WST_AIKillTask task, bool value)
	{
		if(!GetTaskManager())
			return;
	...
	void PopUpNotification(WST_AIKillTask task,string msg)
	{
	...
	WST_AIKillTask CreateNewDeliverTask(Faction faction, vector deliveryDestination,IEntity itemToDeliver ,WST_Type type  )
	{
		if (!GetTaskManager())
			return null;
...
///CHECK PREFAB FOR CORRECT COMPOSITION
		WST_AIKillTask task = WST_AIKillTask.Cast(bTask);
	...
	//------------------------------------------------------------------------------------------------
	void SetTaskType(notnull WST_AIKillTask task, SCR_CampaignTaskType type)
	{
		if (!GetTaskManager())
			return;
	...
	//------------------------------------------------------------------------------------------------
	void WST_AIKillTaskSupportEntity(IEntitySource src, IEntity parent)
	{
	...
	//------------------------------------------------------------------------------------------------
	void ~WST_AIKillTaskSupportEntity()
	{
